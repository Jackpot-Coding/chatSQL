...
\begin{document}
	...
	\begin{flushleft}
		\begin{spacing}{1.5}
			REDATTORI: G. Moretto, M.Gobbo\\%INSERT HERE THE NAMES
			VERIFICATORI:\\
			\vspace{7pt}
		...
		\resizebox{\linewidth}{!}{
			\begin{tabular}{|c|c|c|c|c|}
				...
				\textbf{Versione} & \textbf{Data} & \textbf{Autore} & \textbf{Verificatore} & \textbf{Modifica} \\
    				\hline
				0.0.5 & 04/04/2024 & M. Camillo & - & Introduzione Documento \\
                		\hline
				0.0.4 & 04/04/2024 & M. Gobbo & - & Introduzione all'architettura \\
				\hline
    ...
    Lo scopo di questo documento, \textit{Specifica Architetturale}, è quello di presentare, motivandole, le scelte architetturali e di design che è stato deciso di applicare al prodotto, oltre che a fornire una lista completa delle tecnologie utilizzate. La struttura interna del prodotto è esposta all'interno del dcoumento sotto forma di diagramma delle classe, in maniera da rendere più chiaro il software sviluppato. Vengono inoltre approfonditi e motivati a loro volta i design pattern utilizzati. 
...
In questo modello, il \textit{"Model"} rappresenta i dati e la logica di \textit{business} dell'applicazione, la \textit{"View"} è responsabile della presentazione dei dati all'utente e la \textit{"Template"} definisce la struttura della \textit{View} e come i dati vengono visualizzati al suo interno. Più nello specifico il \textit{"Template"}, nel caso della nostra applicazione, definisce la struttura \textit{HTML}.\\
