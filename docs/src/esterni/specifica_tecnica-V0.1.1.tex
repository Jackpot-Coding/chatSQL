	...
	\begin{table}[H]
		...
		\resizebox{\linewidth}{!}{
			\begin{tabular}{|c|c|c|c|c|}
				...
				\hline
				0.1.2 & 06/05/2024 & G. Moretto & - & Aggiunta CSVParser e miglioramento descrizioni delle classi \\
				\hline
				0.1.1 & 03/05/2024 & E. Gallo & - & Sistemato \textit{Design pattern} utilizzati - \textit{Strategy} \\
				\hline
...
\end{itemize}
	...
	Il compito delle \textit{view} è quella di ricevere richieste dal browser\textsuperscript{G} e di restituire risposte sottoforma di pagine HTML o altri elementi che possono essere visualizzati da un browser.\\
	...
	\begin{itemize}
		\item  \textbf{MainView}: si occupa di ricevere la richiesta di generazione del \textit{prompt}\textsuperscript{G} dall'utente e di restituirlo. Per fare questo utilizza una classe chiamata PromptCreator che si occupa di generare il prompt per la richiesta ricevuta dalla view.
		\item \textbf{PromptCreator}: si occupa di creare il \textit{prompt} a partire dalla struttura database e dalla frase in linguaggio naturale fornita;
		\item \textbf{QueryGenerationView}: si occupa di ricevere il prompt generato, inviarlo al modello \textit{LLM}\textsuperscript{G} esterno tramite la classe Querygenerator e ritornare la risposta;
		\item \textbf{QueryGenerator}: si occupa di generare la query SQL richiesta inviando il \textit{prompt} generato ad un servizio LLM esterno;
	\end{itemize}
	...
	\begin{itemize}
		...
		\item \textbf{FileUploader}: reponsabile per il caricamento di file che rappresentano una struttura database, utilizza l'interfaccia \textbf{ParserStrategy}  che viene implementata con le classi:
		\begin{itemize}
			\item \textbf{JSONParser}: per importare file JSON con struttura nota;
			\item \textbf{CSVParser}: per importare file CSV con struttura nota;
		\end{itemize}
	...
	\subsection{Design pattern utilizzati}
		...
		\subsubsection{Strategy}
			...
			In particolare, viene integrato per permettere al programma di riconoscere diversi tipi di file quando viene eseguito \textit{l'upload}. In base all'estensione del file, il programma sceglie quale strategia utilizzare per raccogliere ed immagazzinare nel migliore dei modi le informazioni caricate.
			Al lato pratico, una volta caricato un file tramite la classe \textit{FileUploader}, questo viene passato ad un'interfaccia che verrà successivamente implementata da le classi:
			\begin{itemize}
				\item \textit{JSONParser}: per i file di tipo JSON
				\item \textit{CSVParser}: per i file CSV
			\end{itemize} 
			Il gruppo ha poi individuato altri ambiti in cui un \textit{Design Pattern Strategy} sarebbe ottimale. Per una questione di tempo principalmente, non sono stati implementati. Siccome il prodotto deve lavorare a stretto contatto con gli LLM\textsuperscript{G}, sarebbe utile che la loro chiamata venga effettuata solo quando necessario, senza usare i modelli più grandi per richieste piccole.  \\
